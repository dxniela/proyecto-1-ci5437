...
void cargarPDBS(){
	...
	FILE *f3 = fopen("TopSpin14-4_abst3.pdb", "r");
 
	abs1 = read_abstraction_from_file("TopSpin14-4_abst1.abst");
	map1 = read_state_map(f1);

	abs2 = read_abstraction_from_file("TopSpin14-4_abst2.abst");
	map2 = read_state_map(f2);

	abs3 = read_abstraction_from_file("TopSpin14-4_abst3.abst");
