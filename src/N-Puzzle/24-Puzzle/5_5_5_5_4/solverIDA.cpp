...
void cargarPDBS(){
	...
	FILE *f5 = fopen("24-Puzzle_abst5.pdb", "r");
	...
	abs5 = read_abstraction_from_file("24-Puzzle_abst5.abst");
...
unsigned heuristicas(state_t puzzle_state){
	...
	unsigned sum = *state_map_get(map1, &abs_state1) + *state_map_get(map2, &abs_state2) + *state_map_get(map3, &abs_state3) + *state_map_get(map4, &abs_state4) + *state_map_get(map5, &abs_state5);
